...
func (m *mockPriceService) GetPriceFor(itemCode string) (float64, error) {
	...
	// just a little change to allowed tests to run concurrently for parallel solution
	// if not, a race condition is produced with m.numCalls var
	mux.Lock()
	m.numCalls++
	mux.Unlock()            // increase the number of calls
...
// I add this unit test to check if parallel calls works well when a goroutine returns an error
func TestGetPricesFor_ParallelizeCallsWithErr(t *testing.T) {
	mockService := &mockPriceService{
		callDelay: time.Second, // each call to external service takes one full second
		mockResults: map[string]mockResult{
			"p1": {price: 5, err: nil},
			"p2": {price: 0, err: errors.New("external price service error")},
			"p3": {price: 7, err: nil},
		},
	}
	cache := NewTransparentCache(mockService, time.Minute)
...
// Simple test to check expired key functionality
func TestRead_ExpiredKey(t *testing.T) {
	cache := NewTransparentCache(&mockPriceService{}, 5*time.Millisecond)
	cache.Write("expired_key", 10)
	time.Sleep(6 * time.Millisecond)
	_, found := cache.Read("expired_key")
	assert.False(t, found)
}

// Simple test to check thet a key must be returned when it's requested before expiration time
func TestRead_NotExpiredKey(t *testing.T) {
	cache := NewTransparentCache(&mockPriceService{}, 1*time.Second)
	cache.Write("key", 10)
